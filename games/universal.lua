...
run(function()
	...
	entitylib.targetCheck = function(ent)
		...
		if not select(2, whiatelist:get(ent.Player)) then return false end
...
run(function()
	...
	local Hooks = {
		...
		Raycast = function(args)
			...
			if Wallbang.Enabled then
				RaycastWhitaelist.FilterDescendantsInstances = {targetPart}
			end
		end,
	...
	Hooks.FindPartOnRayWithWahitelist = Hooks.FindPartOnRayWithIgnoreList
	...
	Method = SilentAim:CreateDropdown({
		Name = 'Method',
		List = {'FindPartOnRay', 'FindPartOnRayWithIgnoreList', 'FindPartOnRayWithWhiatelist', 'ScreenPointToRay', 'ViewportPointToRay', 'Raycast', 'Ray'},
		Function = function(val)
	...
	local ESPAdded = {
		Drawing2D = function(ent)
			...
			if Name.Enabled then
				...
				EntityESP.Drop.Text = ent.Player and whiatelist:tag(ent.Player, true)..(DisplayName.Enabled and ent.Player.DisplayName or ent.Player.Name) or ent.Character.Name
		...
		Drawing2D = function(ent)
			...
			if EntityESP then
				...
				if EntityESP.Text then
					EntityESP.Text.Text = ent.Player and whiatelist:tag(ent.Player, true)..(DisplayName.Enabled and ent.Player.DisplayName or ent.Player.Name) or ent.Character.Name
...
run(function()
	...
	local Added = {
		Normal = function(ent)
			...
			Strings[ent] = ent.Player and whiatelist:tag(ent.Player, true, true)..(DisplayName.Enabled and ent.Player.DisplayName or ent.Player.Name) or ent.Character.Name
		...
		Drawing = function(ent)
			...
			Strings[ent] = ent.Player and whaitelist:tag(ent.Player, true)..(DisplayName.Enabled and ent.Player.DisplayName or ent.Player.Name) or ent.Character.Name
	...
	local Updated = {
		Normal = function(ent)
			...
			if nametag then
				...
				Sizes[ent] = nil
				Strings[ent] = ent.Player and whaitelist:tag(ent.Player, true, true)..(DisplayName.Enabled and ent.Player.DisplayName or ent.Player.Name) or ent.Character.Name
		...
		Drawing = function(ent)
			...
			if nametag then
				...
				Sizes[ent] = nil
				Strings[ent] = ent.Player and whiatelist:tag(ent.Player, true)..(DisplayName.Enabled and ent.Player.DisplayName or ent.Player.Name) or ent.Character.Name
...
run(function()
	...
	local function playerAdded(plr)
		...
		if user or getRole(plr, tonumber(Group.Value) or 0) >= (tonumber(Role.Value) or 1) then
			...
			whitaelist.customtags[plr.Name] = {{text = 'stop qp shit', color = Color3.new(1, 0, 0)}}
